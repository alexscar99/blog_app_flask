...
def logout():
    logout_user()
    flash('You have successfully logged out. Log back in to create a new post or '
          'to modify existing ones', 'success')
    return redirect(url_for('main.index'))
...
def account():
    ...
    img_file = url_for(
        'static', filename=f'profile_pics/{current_user.img_file}')
